...
namespace jquery_ui_fileupload.Controllers
{
    public class UploadController : Controller
    {
        ...
        public ActionResult Upload(IEnumerable<string> names = null) {
            var folder = GetUploadFolder();
            var files = from file in folder.GetFiles() where names == null || names.Contains(file.Name, StringComparer.OrdinalIgnoreCase) select new {
                deleteType = "POST",
                name = file.Name,
                size = file.Length,
                url = Url.Action("GetFile","Home",new { file.Name}),
                thumbnailUrl=Url.Action("GetFile","Home",new { file.Name,thumbnail=true}),
                deleteUrl=Url.Action(Url.Action("DeleteFile","Home",new { file.Name}))
            };
        ...
        public ActionResult Upload() {
            var files = Request.Files.Cast<string>().Select(k => Request.Files[k]).ToArray();
